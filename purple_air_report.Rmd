...
params:
    company: "PurpleAir"
    device: !r c("PA2", "PA3", "PA4", "PA5", "PA6", "PA7", "PA8", "PA9", "PA10")
    pollutant: "PM2.5"
    sensor: "1"
    cal_version: "indoor"
    start: "2019-12-10"
...
regress_f <- function(data1, lcsmeas, refmeas, ref_lcs, deploy){
   ...
   model <- lm(lcsmeas ~ refmeas, data = data1)
    ...
    reg_p <- ggplot(data = data1,
                    mapping = aes(x = refmeas, y = lcsmeas, color = Instrument, numeric = TRUE)) + geom_abline(size=1, colour="gray29", alpha=0.9) + geom_point(alpha = alpha_size, size =
