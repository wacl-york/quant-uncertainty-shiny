...
\fancyhead[R]{\textbf{AQMesh NO2: } \slshape\leftmark}  
...
regress_f <- function(data1, lcsmeas, refmeas, ref_lcs, deploy){
   ...
   model <- lm(lcsmeas ~ refmeas, data = data1)
    ...
    reg_p <- ggplot(data = data1,
                    mapping = aes(x = refmeas, y = lcsmeas, color = Instrument)) +geom_abline(size=1) + geom_point(alpha = alpha_size, size =
                                                                                                  0.1)  +  geom_smooth(method = lm, se = FALSE) +
        annotate(
            geom = "text",
            ,
...
#time_series_f(manch_lcs, lcsmeas, refmeas, deploy)
```
...
#time_series_f(london_lcs, lcsmeas, refmeas, deploy)
```
...
#time_series_f(york_lcs, lcsmeas, refmeas, deploy)
```
