...
#plot_scatter(lcs, lcs_column="lcsmeas", reference_column="refmeas") + coord_cartesian()

ggplot(data=lcs, mapping = aes(x=refmeas, y=lcsmeas)) +geom_point()
...
```{r echo=FALSE}
model <- lm(lcsmeas ~ refmeas, data = lcs)
...
```{r echo=FALSE}
sprintf("y = %.2fx %s %.1f", coef(model)[2], ifelse(sign(coef(model)[1]) == 1, "+", ""), coef(model)[1])
```
